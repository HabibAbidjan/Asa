...
def process_withdraw(message):
    ...
    # Karta raqami faqat raqamlardan iborat va uzunligi mos bo‘lishi kerak
    if not card.isdigit() or len(card) < 12 or len(card) > 19:
        bot.send_message(message.chat.id, "❌ Iltimos, to‘g‘ri karta raqamini yuboring (faqat raqam, 12-19 xonali).")
        return
